...
<form id="procDefForm" style="display:none;" action="${APP_PATH}/process/uploadFile" method="post" enctype="multipart/form-data">
 ...
 <hr style="clear:both;">
    ...
    <script type="text/javascript">
            ...
            $("#procDefFile").change(function(){
            	$("#procDefForm").ajaxSubmit({
            		success: function(result){
            			if(result.success){
            				layer.msg("上传流程定义文件成功", {time:1000, icon:6});
            				queryPage(0);
            			}else{
            				layer.msg("上传流程定义文件失败", {time:1000, icon:5, shift:6});
            			}
            		}
            	}); //异步提交。
            });
            		...
            		success : function(result){
            			...
            			if(result.success){
            				...
            				$.each(list,function(i,e){
        						...
        						content+='  <td>'+e.version+'</td>';
        						content+='  <td>'+e.key+'</td>';
        						content+='  <td>';
            					content+='<button type="button" onclick="window.location.href=\'${APP_PATH}/process/view?pdid='+e.id+'\'" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-eye-open"></i></button>';
            					content+='<button type="button" onclick="deleteProcDef(\''+e.deployid+'\',\''+e.name+'\')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
            					content+='</td>';
            					content+='</tr>';
            				});
            ...
            function deleteProcDef(deployid, pdname) {
                layer.confirm("删除流程定义信息["+pdname+"]，是否继续？",  {icon: 3, title:'提示'}, function(cindex){
                    $.ajax({
                        type : "POST",
                        url  : "${APP_PATH}/process/deleteDeployement",
                        data : {
